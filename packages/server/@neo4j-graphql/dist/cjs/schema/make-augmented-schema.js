...
function getObjFieldMeta(_a) {
    ...
    return (_b = obj === null || obj === void 0 ? void 0 : obj.fields) === null || _b === void 0 ? void 0 : _b.reduce(function (res, field) {
        ...
        var readonly = (field.directives || []).find(function (x) { return x.name.value === "readonly"; });
        var baseField = {
            fieldName: field.name.value,
            typeMeta: typeMeta,
            otherDirectives: (field.directives || []).filter(function (x) { return !["relationship", "cypher", "autogenerated", "readonly"].includes(x.name.value); }),
...
function makeAugmentedSchema(options) {
    ...
    neoSchemaInput.nodes.forEach(function (node) {
        ...
        var nodeUpdateInput = composer.createInputTC({
            ...
            fields: __spread(node.primitiveFields, node.scalarFields, node.enumFields, node.dateTimeFields).filter(function (x) { return !x.readonly; })
        ...
        if (node.relationFields.length) {
            if (node.relationFields.filter(function (x) { return !x.readonly; }).length) {
                _a = __read(["ConnectInput", "DisconnectInput"].map(function (type) {
                    return composer.createInputTC({
                        name: "" + node.name + type,
                        fields: {},
                    });
                }), 2), nodeConnectInput = _a[0], nodeDisconnectInput = _a[1];
            }
        ...
        composer.createInputTC({
            ...
            fields: __assign({ where: node.name + "Where" }, (node.relationFields.filter(function (x) { return !x.readonly; }).length ? { connect: nodeConnectInput } : {})),
        });
        composer.createInputTC({
            name: node.name + "DisconnectFieldInput",
            fields: __assign({ where: node.name + "Where" }, (node.relationFields.filter(function (x) { return !x.readonly; }).length ? { disconnect: nodeDisconnectInput } : {})),
        });
        node.relationFields.forEach(function (rel) {
            ...
            if (rel.union) {
                ...
                composeNode.addFields((_a = {},
                    ...
                    _a));
                refNodes.forEach(function (refNode) {
                    var _a, _b, _c, _d, _e, _f;
                    composeNode.addFieldArgs(rel.fieldName, (_a = {},
                        _a[refNode.name] = refNode.name + "Where",
                        _a));
                    var concatFieldName = rel.fieldName + "_" + refNode.name;
                    var createField = rel.typeMeta.array
                        ? "[" + refNode.name + "CreateInput]"
                        : refNode.name + "CreateInput";
                    var updateField = refNode.name + "UpdateInput";
                    var nodeFieldInputName = "" + node.name + utils_1.upperFirstLetter(rel.fieldName) + refNode.name + "FieldInput";
                    var nodeFieldUpdateInputName = "" + node.name + utils_1.upperFirstLetter(rel.fieldName) + refNode.name + "UpdateFieldInput";
            ...
            var refNode = neoSchemaInput.nodes.find(function (x) { return x.name === rel.typeMeta.name; });
            ...
            [whereInput, andInput, orInput].forEach(function (inputType) {
                ...
                inputType.addFields((_a = {},
                    _a[rel.fieldName] = refNode.name + "Where",
                    _a[rel.fieldName + "_NOT"] = refNode.name + "Where",
                    _a[rel.fieldName + "_IN"] = "[" + refNode.name + "Where]",
                    _a[rel.fieldName + "_NOT_IN"] = "[" + refNode.name + "Where]",
                    _a));
            });
            var createField = rel.typeMeta.array ? "[" + refNode.name + "CreateInput]" : refNode.name + "CreateInput";
            var updateField = refNode.name + "UpdateInput";
            ...
            nodeInput.addFields((_c = {},
                _c[rel.fieldName] = nodeFieldInputName,
                _c));
            nodeRelationInput.addFields((_d = {},
                _d[rel.fieldName] = createField,
                _d));
            if (rel.readonly) {
                return;
            }
