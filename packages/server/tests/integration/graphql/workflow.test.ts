...
describe("workflow", () => {
    ...
    test("should allow user to create blog edit the blog, create a post, edit the post, create a comment, edit the comment and delete everything", async () => {
        ...
        try {
            ...
            `);
            expect(createComments.comments[0].content).toEqual(
            ...
            `);
            expect(updateComments.comments[0].content).toEqual(
                comment.updatedContent
            );

            await mutate(gql`
                mutation {
                    deleteComments(where: { id: "${comment.id}" }) {
                        nodesDeleted
                    }
                }
            `);
